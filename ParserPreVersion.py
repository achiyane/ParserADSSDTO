...
def createServiceFiles():
    ...
    for i in fD.keys():
        ...
        open(f'{ap}\\S{i}.java', "w").write(fin)
...
def createDTOFiles():
    ...
    for i in fD.keys():
        ...
        open(f'{ap}\\{i + "DTO.java"}', "w").write(fin)
...
def createDAOFiles():
    ...
    fD = getFieldDict()
    for i in fD.keys():
        fin = createDAOClass(i, fD)
        open(f'{ap}\\{i + "DAO.java"}', "w").write(fin)


def createDAOClass(name, fDict: dict):
...
def DAOsFieldsBasedOnFK(fkDict: dict, name):
    nD = fkDict[name]
    args1 = ""
    addedFields = {}
    addedMethods = {}
...
if __name__ == '__main__':
    test = input("Test or Not? [y/n]\n")
    test = test != "n"
    if test:
        PathToDB = "1"
        if PathToDB != "1":
            open("PathFiles/dbPath.txt", "w").write(PathToDB)
        else:
            PathToDB = open("PathFiles/dbPath.txt").read().strip()
        pathToSrc = "0"
        if pathToSrc == "0":
            pathToSrc = "0"
        elif pathToSrc == "1":
            pathToSrc = open("PathFiles/srcPath.txt").read().strip()
        else:
            open("PathFiles/srcPath.txt", "w").write(pathToSrc)
        pathToDal = "2"
        if pathToDal == "1":
            pathToDal = open("PathFiles/dalPath.txt").read().strip()
        elif pathToDal == "2":
            pathToDal = "1"
        else:
            open("PathFiles/dalPath.txt", "w").write(pathToDal)
        backend_dal = "Backend"
        s = "none"
        createDAOFiles()
        createBusinessFiles()
